...
def permuted(signal):
    ...
    segment_length = len(signal) // 5  # 100 for 500-length signals
    sig = signal[listA[0]*segment_length : listA[0]*segment_length + segment_length]
    for i in range(1, len(listA)):
        seg = signal[listA[i]*segment_length : listA[i]*segment_length + segment_length]
        sig = np.vstack((sig, seg))
    return sig
...
def shuffle(normal_s_raw,normal_s_no,normal_s_ne,normal_s_op,normal_s_pe,normal_s_sc,normal_s_ti):
    path = "/content/AMSL/data/transform_dataset/train_dataset/"
    ######################shuffle data#########################
...
if __name__ == '__main__':
    normal = np.load("/content/AMSL/data/generate_dataset/normal.npy")
    abnormal = np.load("/content/AMSL/data/generate_dataset/abnormal.npy")
    ...
    ####################################save abnormal data######################
    path = '/content/AMSL/data/transform_dataset/test_dataset/'
